...
public class WorldGame {
	...
	final public float TIME_TO_CHANGE_STATE_PLATAFORM = 16f; // Este tiempo debe ser mayor que DURATION_ACTIVE en la clase plataformas.
	public float timeToChangeStatePlataform;

	final public float TIME_TO_SPWAN_COIN = .75f;
	...
	public void update(float delta, float acelX, boolean slam) {
		...
		timeToChangeStatePlataform += delta;
		...
		if (timeToSpawnCoin >= TIME_TO_SPWAN_COIN) {
			timeToSpawnCoin -= TIME_TO_SPWAN_COIN;
			crearMonedas();
		}

		if (timeToChangeStatePlataform >= TIME_TO_CHANGE_STATE_PLATAFORM) {
			timeToChangeStatePlataform -= TIME_TO_CHANGE_STATE_PLATAFORM;
			if (oRan.nextBoolean()) {
